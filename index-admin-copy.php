...
<script>
    ...
    var scholarsBarChart = new Chart(ctx, {
        ...
        options: {
            ...
            onClick: (e) => {
                ...
                var activePoints = scholarsBarChart.getElementsAtEventForMode(e, 'nearest', { intersect: true }, true);
                if (activePoints.length) {
                    var index = activePoints[0].index; // Index of the clicked bar
                    var selectedSchoolYear = schoolYears[index];

                    // Fetch drill-down data via AJAX
                    fetchDrillDownData(selectedSchoolYear);
                }
            }
        }
    });

    function fetchDrillDownData(selectedSchoolYear) {
        // AJAX request to fetch drill-down data
        $.ajax({
            url: 'contoller/bardrilldown.php', // Replace with the actual file that handles drill-down queries
            type: 'POST',
            data: { schoolYear: selectedSchoolYear },
            success: function (response) {
                try {
                    var detailedData = JSON.parse(response);

                    if (detailedData.labels && detailedData.counts) {
                        // Update the chart with the drill-down data
                        updateScholarsBarGraphForDrillDown(detailedData, selectedSchoolYear);
                    } else {
                        alert(`No additional details available for ${selectedSchoolYear}.`);
                    }
                } catch (error) {
                    console.error('Error parsing drill-down response:', error);
    ...
    function updateScholarsBarGraphForDrillDown(detailedData, selectedSchoolYear) {
        // Prepare data for the drill-down chart
        var labels = detailedData.labels;
        var counts = detailedData.counts;

        // Destroy the existing chart instance
        scholarsBarChart.destroy();

        // Create a new bar chart with drill-down data
        scholarsBarChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: labels,
                datasets: [
                    {
                        label: `Details for ${selectedSchoolYear}`,
                        data: counts,
